...
********************************************************************************/
//#include "config/.h"
//#include "src/pro/toNN.h"
//#include "src/tlcfea/turizer.h"
//#include "src/utils.h"/
//#include "src/featureextr/action.h"
//#include "src/predictionfilt/er.h"
...
float (*normalize_inp_test_normalize_normAcc_p)[3]= (float (*)[3]) &__vtmem[2408];
...
int16_t (*mpu_acq_test_mpu_acq_gyr_p)[3]= (int16_t (*)[3]) &__vtmem[2408];
#define mpu_acq_test_mpu_acq_gyr (*mpu_acq_test_mpu_acq_gyr_p)
float (*tlcfeaturizer_test_featureVectorF_p)[124]= (float (*)[124]) &__vtmem[2411];
#define tlcfeaturizer_test_featureVectorF (*tlcfeaturizer_test_featureVectorF_p)
uint8_t (*mpu_acq_test_fifoBuffer_p)[64]= (uint8_t (*)[64]) &__vtmem[2411];
#define mpu_acq_test_fifoBuffer (*mpu_acq_test_fifoBuffer_p)
int16_t (*mpu_acq_test_mpu_acq_acc_p)[3]= (int16_t (*)[3]) &__vtmem[2411];
#define mpu_acq_test_mpu_acq_acc (*mpu_acq_test_mpu_acq_acc_p)
float (*protonn_test_predict_x_cap_p)[10]= (float (*)[10]) &__vtmem[2549];
...
//   /         numNewReadings=0;
        ...
        {
//          Serial.println("Entering MPUACQ...");
            if((mpu_acq_test_mpu_acq_fifoCount>=mpu_acq_test_fifoPacketSize))
            {
              //Serial.println("FifoCount>FifoPacketSize");
...
void _normalize_inp_test_minMaxNormalize(int16_t normalize_inp_test_minMaxNormalize_v[3], int16_t normalize_inp_test_minMaxNormalize_vmin[3], int16_t normalize_inp_test_minMaxNormalize_vmax[3], float normalize_inp_test_minMaxNormalize_dst[3])
{
    ...
    Serial.println(normalize_inp_test_minAcc[normalize_inp_test_x]);
    }
}
...
{
    {
        ...
        for(int __i=0; __i<124; __i++)
        {
            ...
            Serial.println(tlcfeaturizer_test_featureVectorF[__i]);
        }
...
//    Serial.println(protonn_test_predict_y_cap[0]);
}
void loop()
{
    switch(__state)
